...
static inline double clamp_imu_predict_dt(double dt)
{
  const double kMax = 0.02;
  if (dt > kMax) return kMax;
  if (dt < -kMax) return -kMax;
  return dt;
}
...
class ImuProcess
{
 public:
  ...
  void set_gravity_align(const bool &en);
...
void ImuProcess::set_gravity_align(const bool &en)
{
  gravity_align_en_ = en;
}
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  // 重力对齐: 世界系 Z 轴旋到竖直, 地图地面水平; global_reloc 只搜 yaw 依赖此项
  if (gravity_align_en_ && N > MAX_INI_COUNT)
  {
    const V3D z_up = mean_acc.normalized();
    if (z_up.norm() > 0.5)
    {
      const Eigen::Quaterniond q =
        Eigen::Quaterniond::FromTwoVectors(z_up, Eigen::Vector3d::UnitZ());
      init_state.rot = SO3(q.toRotationMatrix());
    }
  }
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    double dt_pred = clamp_imu_predict_dt(dt);
    kf_state.predict(dt_pred, Q, in);
  ...
  double dt_end = clamp_imu_predict_dt(dt);

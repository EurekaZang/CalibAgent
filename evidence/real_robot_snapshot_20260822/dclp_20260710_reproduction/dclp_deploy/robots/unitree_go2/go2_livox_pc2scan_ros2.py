...
def latest_point_stamp(msg, header_stamp=None):
    """Return the newest per-point Livox timestamp, falling back to the header.

    Livox PointCloud2 messages on this robot store an absolute nanosecond clock
    in a float64 field named ``timestamp``.  The ROS header marks the first
    point in the roughly 50 ms packet, so forwarding the header makes every
    LaserScan look 50 ms older than its newest observation.
    """
    header_stamp = stamp_to_sec(msg.header.stamp) if header_stamp is None else float(header_stamp)
    field = next(
        (
            item
            for item in msg.fields
            if str(item.name) == "timestamp" and int(item.datatype) == 8 and int(item.count) == 1
        ),
        None,
    )
    count = int(msg.width * msg.height)
    if field is None or count <= 0 or int(msg.point_step) < int(field.offset) + 8:
        return header_stamp
    dtype = np.dtype(
        {
            "names": ["timestamp"],
            "formats": [(">" if msg.is_bigendian else "<") + "f8"],
            "offsets": [int(field.offset)],
            "itemsize": int(msg.point_step),
        }
    )
    try:
        values = np.frombuffer(msg.data, dtype=dtype, count=count)["timestamp"]
    except ValueError:
        return header_stamp
    finite = values[np.isfinite(values)]
    if finite.size == 0:
        return header_stamp
    raw_latest = float(np.max(finite))
    for scale in (1.0, 1e-3, 1e-6, 1e-9):
        candidate = raw_latest * scale
        if header_stamp - 0.001 <= candidate <= header_stamp + 1.0:
            return max(header_stamp, candidate)
    return header_stamp


def set_stamp_from_sec(stamp, seconds):
    whole = int(math.floor(float(seconds)))
    nanoseconds = int(round((float(seconds) - whole) * 1e9))
    if nanoseconds >= 1000000000:
        whole += 1
        nanoseconds -= 1000000000
    stamp.sec = whole
    stamp.nanosec = max(0, nanoseconds)
...
class Go2LivoxPc2Scan(Node):
    ...
    def _cloud_cb(self, msg):
        ...
        stamp = latest_point_stamp(msg, header_stamp)
        ...
        scan = LaserScan()
        set_stamp_from_sec(scan.header.stamp, stamp)
        ...
        if self.cloud_count == 1 or self.cloud_count % 50 == 0:
            ...
            packet_span_ms = max(0.0, (stamp - header_stamp) * 1000.0)
            self.get_logger().info(
                ...
                "extract=%.1fms project=%.1fms build_pub=%.1fms total=%.1fms "
                "packet_span=%.1fms latest_point→scan_pub=%.1fms"
                % (
                    self.publish_count,
                    self.cloud_count,
                    msg.header.frame_id,

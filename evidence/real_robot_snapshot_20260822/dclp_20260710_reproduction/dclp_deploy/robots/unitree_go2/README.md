...
- The policy output is `tanh(mu)`; the node directly multiplies it by the DCLP speed scales. With compensation enabled, the resulting `(v,w)` pair is passed once through `velocity_compensation.py`. No deployment-layer floor, acceleration limiter, hard cap, guard, or ZMQ clamp changes the command.
...
| `POLICY_OBS_MAX_LINEAR_ACC` | `2.0` | Linear-acceleration value in the policy observation tail only |
| `POLICY_OBS_MAX_ANGULAR_ACC` | `2.0` | Angular-acceleration value in the policy observation tail only |
| `POLICY_COMPENSATE` | `0` | `1` directly applies the calibration function; `0` bypasses it |

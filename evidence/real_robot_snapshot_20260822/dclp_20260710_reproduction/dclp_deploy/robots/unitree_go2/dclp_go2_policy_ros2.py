...
class DclpGo2PolicyRos2(Node):
    def __init__(self):
        ...
        self.scheduler_rate_hz = env_float(
            "POLICY_SCHEDULER_RATE_HZ", max(self.control_rate_hz, 100.0)
        )
        ...
        self.compensation_mode = "calibration_direct" if self.use_compensation else "off"
        self.action_mapping_mode = "policy_direct_scale"
        ...
        self.robot = DclpRobotContext(
            ...
            max_linear_acc=env_float("POLICY_OBS_MAX_LINEAR_ACC", 2.0),
            max_angular_acc=env_float("POLICY_OBS_MAX_ANGULAR_ACC", 2.0),
        )
        ...
        if self.use_compensation and not _HAS_COMPENSATION:
            raise RuntimeError(
                "calibration requested but velocity_compensation.py is unavailable"
            )
        ...
        self.timer = self.create_timer(
            1.0 / max(self.scheduler_rate_hz, 1e-3), self._control_loop
        )
        self.get_logger().info(
            "Go2 DCLP policy ready: backend=%s scan=%s odom=%s goal=%s rel_goal=%s global=%s zmq=%s rate=%.1fHz scan_stamp_timeout=%.2fs experiment=%s profile=%s compensation=%s mapping=%s config=%s"
            % (
    ...
    def _build_effective_config(self):
        ...
        return {
            ...
            "command_path": "policy_direct_scale_then_optional_calibration",
            ...
            "scheduler_rate_hz": self.scheduler_rate_hz,
            "input_sync_mode": "new_scan_and_new_odom",
            ...
            "policy_observation_max_linear_acc": self.robot.max_linear_acc,
            "policy_observation_max_angular_acc": self.robot.max_angular_acc,
    ...
    def _odom_cb(self, msg):
        with self.lock:
            ...
            self.latest_odom_seq = self._odom_rx_count
    ...
    def _apply_compensation(self, desired_cmd):
        ...
        return calibrated, calibrated.copy(), (True, True), ("calibration_direct", "calibration_direct")
    ...
    def _control_loop_impl(self):
        ...
        if not ready:
            ...
            return
        # A policy action must use a newly arrived scan and a newly arrived
        # odometry sample.  Without this pairwise gate the independent 25 Hz
        # timer can consume scan N with odometry N-1, adding a full 50 ms LIO
        # period to the action's reference age while still looking "fresh" by
        # receive time.  Leave both sequence markers untouched until the pair
        # is available so the next timer tick can consume it.
        if (
            scan_seq == self.last_control_scan_seq
            or odom_seq == self.last_control_odom_seq
        ):
            return
        self.last_control_scan_seq = scan_seq
        self.last_control_odom_seq = odom_seq
        ...
        desired_cmd = target_cmd.copy()
        cmd, raw_comp, comp_accepted, comp_reasons = self._apply_compensation(
            desired_cmd
        )

...
launch_policy() {
  ...
  export POLICY_MAX_LINEAR="3.0"; export POLICY_MAX_ANGULAR="3.1416"
  export POLICY_CMD_VEL_V_CAP="3.0"; export POLICY_CMD_VEL_W_CAP="3.1416"
  # Let the policy publish continuous speeds instead of snapping non-zero commands to fixed floors.
  export POLICY_CMD_VEL_V_FLOOR="0.0"; export POLICY_CMD_VEL_W_FLOOR="0.0"
...
launch_zmq() {
  ...
  exec python3 "${ROBOT_DIR}/go2_zmq_sport_client.py" \
    ...
    --max-linear 3.0 --max-angular 3.1416 --allow-reverse --sport-timeout 5.0
}

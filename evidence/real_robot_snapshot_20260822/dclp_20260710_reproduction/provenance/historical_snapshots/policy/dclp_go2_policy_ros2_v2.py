...
class DclpGo2PolicyRos2(Node):
    def __init__(self):
        ...
        self.cmd_vel_v_floor = env_float("POLICY_CMD_VEL_V_FLOOR", 0.0)
        self.cmd_vel_w_floor = env_float("POLICY_CMD_VEL_W_FLOOR", 0.0)
    ...
    def _control_loop_impl(self):
        ...
        # Use the previous command for command-side acceleration limiting. On the
        # real Go2, tiny commands may not overcome the motion deadband, so odom
        # can stay near zero and otherwise pin the limiter to one small step.
        cmd = normalized_action_to_cmd_vel(
            normalized_action=normalized_action,
            current_vw=self.last_cmd,
            robot=self.robot,
        ...
        cmd = self._clip_cmd((v, w), apply_w_floor=not straighten_front_goal)

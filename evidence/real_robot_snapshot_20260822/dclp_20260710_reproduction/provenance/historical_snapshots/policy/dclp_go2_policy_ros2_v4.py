...
class DclpGo2PolicyRos2(Node):
    ...
    def _keep_compensation_in_range(desired, compensated, min_speed, max_speed):
        ...
        if comp_mag < lo or comp_mag > hi:
            return max(-hi, min(hi, desired))
        return sign * comp_mag
    ...
    def _control_loop_impl(self):
        ...
        # Only accept compensated values that remain inside the configured speed
        # range; otherwise keeping the policy-scaled value avoids cap saturation
        # turning continuous commands into long constant segments.
        if self.use_compensation and _HAS_COMPENSATION:
            compensated_cmd = self._compensate_cmd_in_range((v, w))

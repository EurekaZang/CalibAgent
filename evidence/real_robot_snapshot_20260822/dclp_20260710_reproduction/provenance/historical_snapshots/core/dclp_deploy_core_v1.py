...
def pool_scan_to_dclp_groups(
    *,
    ...
    if np.any(finite):
        ...
        # Keeping pooled distances in the same order as [cos(center), sin(center)]
        # is required by the trained policy observation contract.
        shifted = (wrapped_angles + np.float32(np.pi / 2.0)) % np.float32(2.0 * np.pi)

...
and converted to physical cmd_vel with DCLP's acceleration-limited control.
"""
...
def pool_scan_to_dclp_groups(
    *,
    ...
    if np.any(finite):
        ...
        # The policy-facing sector distribution is rotated clockwise by 180 degrees
        # while keeping the trained policy's [cos(center), sin(center)] contract.
        shifted = (
            wrapped_angles + np.float32(np.pi / 2.0) - np.float32(np.pi)
...
def clip_normalized_action(action: Sequence[float]) -> np.ndarray:
    arr = np.asarray(action, dtype=np.float32).reshape(-1)
    if arr.shape[0] < 2:
        raise ValueError("DCLP normalized action must contain at least 2 values, got %d" % arr.shape[0])
    return np.clip(arr[:2], -1.0, 1.0).astype(np.float32, copy=False)
...
def normalized_action_to_cmd_vel(
    *,
    ...
    action = clip_normalized_action(normalized_action)
    current = np.asarray(current_vw, dtype=np.float32).reshape(-1)
    if current.shape[0] < 2:
        raise ValueError("current_vw must contain linear and angular speed")
    period = max(float(control_period_sec), 0.0)
    target_v = float(action[0]) * float(robot.max_linear_speed)
    target_w = float(action[1]) * float(robot.max_angular_speed)
    next_v = np.clip(
        target_v,
        float(current[0]) - float(robot.max_linear_acc) * period,
        float(current[0]) + float(robot.max_linear_acc) * period,
    )
    next_w = np.clip(
        target_w,
        float(current[1]) - float(robot.max_angular_acc) * period,
        float(current[1]) + float(robot.max_angular_acc) * period,
    )
    return np.asarray([next_v, next_w], dtype=np.float32)

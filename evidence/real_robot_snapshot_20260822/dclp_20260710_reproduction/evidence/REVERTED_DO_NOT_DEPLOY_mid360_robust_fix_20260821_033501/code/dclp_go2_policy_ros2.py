...
class DclpGo2PolicyRos2(Node):
    def __init__(self):
        ...
        self.scan_min_valid_beams = max(1, env_int("POLICY_SCAN_MIN_VALID_BEAMS", 64))
        self.front_safety_enabled = env_bool("POLICY_FRONT_SAFETY_ENABLED", True)
        self.front_safety_angle = max(0.0, env_float("POLICY_FRONT_SAFETY_ANGLE", 0.55))
        self.front_safety_base_distance = max(
            0.0, env_float("POLICY_FRONT_SAFETY_BASE_DISTANCE", 0.55)
        )
        self.front_safety_reaction_time = max(
            0.0, env_float("POLICY_FRONT_SAFETY_REACTION_TIME", 0.12)
        )
        self.front_safety_decel = max(0.1, env_float("POLICY_FRONT_SAFETY_DECEL", 3.0))
        self.front_safety_release_margin = max(
            0.0, env_float("POLICY_FRONT_SAFETY_RELEASE_MARGIN", 0.15)
        )
        self.front_safety_hold_sec = max(
            0.0, env_float("POLICY_FRONT_SAFETY_HOLD_SEC", 0.75)
        )
        self.front_safety_min_cluster_beams = max(
            1, env_int("POLICY_FRONT_SAFETY_MIN_CLUSTER_BEAMS", 2)
        )
        self.front_safety_reverse_w_limit = max(
            0.0, env_float("POLICY_FRONT_SAFETY_REVERSE_W_LIMIT", 0.35)
        )
        ...
        self.robot = DclpRobotContext(
            length1=env_float("DCLP_LENGTH1", 0.6048),
            length2=env_float("DCLP_LENGTH2", 0.6048),
            width=env_float("DCLP_WIDTH", 0.53568),
        ...
        self.front_safety_latched_until = 0.0
        self.front_safety_last_distance = float("inf")
        self.front_safety_active = False
    ...
    def _build_effective_config(self):
        ...
        return {
            ...
            "scan_min_valid_beams": self.scan_min_valid_beams,
            ...
            "front_safety_enabled": self.front_safety_enabled,
            "front_safety_angle": self.front_safety_angle,
            "front_safety_base_distance": self.front_safety_base_distance,
            "front_safety_reaction_time": self.front_safety_reaction_time,
            "front_safety_decel": self.front_safety_decel,
            "front_safety_release_margin": self.front_safety_release_margin,
            "front_safety_hold_sec": self.front_safety_hold_sec,
            "front_safety_min_cluster_beams": self.front_safety_min_cluster_beams,
            "front_safety_reverse_w_limit": self.front_safety_reverse_w_limit,
    ...
    def _scan_cb(self, msg):
        valid_beams = self._valid_scan_beam_count(msg)
        with self.lock:
        ...
        if count == 1:
            self.get_logger().info(
                "received first scan: frame=%s beams=%d valid=%d"
                % (msg.header.frame_id, len(msg.ranges), valid_beams)
            )

    @staticmethod
    def _valid_scan_beam_count(scan_msg):
        raw = np.asarray(getattr(scan_msg, "ranges", ()), dtype=np.float32).reshape(-1)
        if raw.size == 0:
            return 0
        range_min = float(getattr(scan_msg, "range_min", 0.0))
        range_max = float(getattr(scan_msg, "range_max", float("inf")))
        valid = np.isfinite(raw) & (raw >= np.float32(range_min)) & (raw <= np.float32(range_max))
    ...
    def _scan_points_in_base(self, scan_msg):
        ...
        valid = (
            np.isfinite(raw)
            & (raw >= np.float32(scan_range_min))
            & (raw <= np.float32(scan_range_max))
        )
        if not np.any(valid):
            return np.zeros(0, dtype=np.float32), np.zeros(0, dtype=np.float32)
        ...
        sensor_angles = (
            ...
            + np.flatnonzero(valid).astype(np.float32) * float(scan_msg.angle_increment)
        )
        ...
        try:
            self._trajectory_log_writer.writerow(
                [
                    ...
                    int(cmd_diag.get("scan_valid_beams", -1)),
                    int(bool(cmd_diag.get("front_safety_active", False))),
                    int(bool(cmd_diag.get("front_safety_hard_brake", False))),
                    "%.6f" % float(cmd_diag.get("front_obstacle_distance", float("nan"))),
                    "%.6f" % float(cmd_diag.get("front_stop_threshold", float("nan"))),
                ]
            )
    ...
    def _ready(self, now, scan_msg, scan_time, odom_msg, odom_time, goal_msg, goal_time):
        ...
        valid_beams = self._valid_scan_beam_count(scan_msg)
        if valid_beams < self.scan_min_valid_beams:
            return (
                False,
                "WAITING_FOR_SCAN",
                "scan valid beams %d < %d" % (valid_beams, self.scan_min_valid_beams),
            )
    ...
    def _clustered_front_distance(self, ranges, angles, max_distance):
        ranges_arr = np.asarray(ranges, dtype=np.float32).reshape(-1)
        angles_arr = wrap_to_pi(np.asarray(angles, dtype=np.float32).reshape(-1))
        if ranges_arr.shape != angles_arr.shape or ranges_arr.size == 0:
            return float("inf")
        valid = (
            np.isfinite(ranges_arr)
            & np.isfinite(angles_arr)
            & (ranges_arr > 0.0)
            & (ranges_arr <= np.float32(max_distance))
            & (np.abs(angles_arr) <= np.float32(self.front_safety_angle))
        )
        if not np.any(valid):
            return float("inf")
        candidate_angles = angles_arr[valid]
        candidate_ranges = ranges_arr[valid]
        order = np.argsort(candidate_angles)
        candidate_angles = candidate_angles[order]
        candidate_ranges = candidate_ranges[order]
        required = self.front_safety_min_cluster_beams
        if required <= 1:
            return float(np.min(candidate_ranges))
        # Accumulated /scan resolution is ~0.33 deg. Allow one missing beam in a
        # cluster while still rejecting isolated dust returns.
        max_gap = max(0.02, 3.0 * 2.0 * math.pi / 1081.0)
        start = 0
        best = float("inf")
        for index in range(1, candidate_angles.size + 1):
            split = index == candidate_angles.size
            if not split:
                split = float(candidate_angles[index] - candidate_angles[index - 1]) > max_gap
            if split:
                if index - start >= required:
                    best = min(best, float(np.min(candidate_ranges[start:index])))
                start = index
        return best

    def _front_safety_state(self, ranges, angles, current_vw, now, planned_v=0.0):
        if not self.front_safety_enabled:
            self.front_safety_active = False
            return {
                "active": False,
                "distance": float("inf"),
        ...
        measured_v = float(np.asarray(current_vw).reshape(-1)[0])
        speed = max(0.0, measured_v, float(self.last_cmd[0]), float(planned_v))
        threshold = (
            self.front_safety_base_distance
            + speed * self.front_safety_reaction_time
            + speed * speed / (2.0 * self.front_safety_decel)
        )
        release_distance = threshold + self.front_safety_release_margin
        distance = self._clustered_front_distance(ranges, angles, release_distance)
        hazard = distance < threshold
        if hazard:
            self.front_safety_active = True
            self.front_safety_latched_until = max(
                self.front_safety_latched_until, now + self.front_safety_hold_sec
            )
        elif self.front_safety_active:
            if now >= self.front_safety_latched_until and distance >= release_distance:
                self.front_safety_active = False
        self.front_safety_last_distance = distance
        return {
            "active": bool(self.front_safety_active),
    ...
    def _apply_front_safety_to_cmd(self, cmd, target_cmd, front_safety):
        safe_cmd = np.asarray(cmd, dtype=np.float32).copy()
        if not front_safety.get("active", False):
            return safe_cmd, False
        target = np.asarray(target_cmd, dtype=np.float32).reshape(-1)
        if float(safe_cmd[0]) > 0.0 or float(target[0]) >= 0.0:
            return np.zeros(2, dtype=np.float32), True
        safe_cmd[0] = min(float(safe_cmd[0]), 0.0)
        if self.front_safety_reverse_w_limit > 0.0:
            safe_cmd[1] = np.clip(
                safe_cmd[1],
                -self.front_safety_reverse_w_limit,
                self.front_safety_reverse_w_limit,
            )
        return safe_cmd, False
    ...
    def _control_loop_impl(self):
        ...
        scan_diag["raw_valid_beams"] = int(ranges.size)
        ...
        front_safety = self._front_safety_state(
            ranges,
            angles,
            current_vw,
            now,
            planned_v=max(float(cmd[0]), float(target_cmd[0])),
        )
        hard_brake = False
        if front_safety["active"]:
            # A positive accel-limited command may remain after the policy has
            # already selected reverse. Bypass the normal limiter and command
            # zero immediately; subsequent negative commands can back away.
            cmd, hard_brake = self._apply_front_safety_to_cmd(
                cmd, target_cmd, front_safety
            )
            self._warn_throttle(
                "front_safety",
                "front safety active: obstacle=%.3fm threshold=%.3fm speed=%.3fm/s "
                "target_v=%.3f cmd_v=%.3f hard_brake=%s"
                % (
                    front_safety["distance"],
                    front_safety["threshold"],
                    front_safety["speed"],
                    float(target_cmd[0]),
                    float(cmd[0]),
                    hard_brake,
        ...
        self._send_cmd(
            ...
            loop_start_mono=now,
            extra_meta={
                "scan_valid_beams": int(scan_diag["raw_valid_beams"]),
                "front_safety_active": bool(front_safety["active"]),
                "front_safety_hard_brake": bool(hard_brake),
                "front_obstacle_distance": float(front_safety["distance"]),
                "front_stop_threshold": float(front_safety["threshold"]),
            },
        )
        self.last_cmd = cmd.copy()
        self.last_control_time = now
        loop_status = "SAFETY_BRAKE" if front_safety["active"] else "NAVIGATING"
        self._set_status(loop_status)
        self._write_trajectory_sample(
            now,
            goal_msg,
            goal_in_base,
            ...
            cmd_diag={
                ...
                "scan_valid_beams": scan_diag["raw_valid_beams"],
                "front_safety_active": front_safety["active"],
                "front_safety_hard_brake": hard_brake,
                "front_obstacle_distance": front_safety["distance"],
                "front_stop_threshold": front_safety["threshold"],
            },
        )
        ...
        if self._loop_count % 10 == 1:
            self.get_logger().info(
                ...
                "(tf=%.1f feat=%.1f infer=%.1f conv=%.1f) | e2e_scan→cmd=%.1fms | "
                "valid=%d cov=%d/90 safety=%s front=%.3f/%.3f"
                % (
                    ...
                    e2e_ms,
                    scan_diag["raw_valid_beams"],
                    scan_diag["coverage_count"],
                    "ON" if front_safety["active"] else "off",
                    front_safety["distance"],
                    front_safety["threshold"],
                ),
            )

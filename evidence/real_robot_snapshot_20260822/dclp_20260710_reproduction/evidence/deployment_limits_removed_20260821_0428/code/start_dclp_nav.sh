...
launch_policy() {
  ...
  # DCLP action is scaled directly by these policy speed parameters.
  export POLICY_MAX_LINEAR="${POLICY_MAX_LINEAR:-0.90}"
  export POLICY_MAX_ANGULAR="${POLICY_MAX_ANGULAR:-0.83776}"
  # These two values are policy observation features only. They do not limit commands.
  export POLICY_OBS_MAX_LINEAR_ACC="${POLICY_OBS_MAX_LINEAR_ACC:-3.0}"
  export POLICY_OBS_MAX_ANGULAR_ACC="${POLICY_OBS_MAX_ANGULAR_ACC:-3.0}"
  ...
  export DCLP_LENGTH1="${DCLP_LENGTH1:-0.504}"
  export DCLP_LENGTH2="${DCLP_LENGTH2:-0.504}"
  export DCLP_WIDTH="${DCLP_WIDTH:-0.4464}"
...
launch_zmq() {
  ...
  exec python3 "${ROBOT_DIR}/go2_zmq_sport_client.py" \
    ...
    --sport-timeout "${GO2_SPORT_TIMEOUT:-5.0}"
}
...
send_goal_and_wait() {
  ...
  if [[ "${DRY_RUN:-0}" == "1" || "${ZERO_MOTION:-0}" == "1" ]]; then
    ...
    if (( fresh_count == 0 )); then
      err "未收到带 action_age 的策略动作"
      return 1
    fi
    info "已记录 ${fresh_count} 条 action age 遥测；ZMQ 不丢弃或改写 policy 动作。"
